...
class Abilities(private val flag: Byte, private val speed: Float, private val field: Float) : MoeAbstractPacket() {
    override fun write(buf: ByteBuf, direction: ProtocolConstants.Direction?, protocolVersion: Int) {
        buf.writeByte(flag.toInt()); listOf(speed, field).forEach { buf.writeFloat(it) }
    }
}