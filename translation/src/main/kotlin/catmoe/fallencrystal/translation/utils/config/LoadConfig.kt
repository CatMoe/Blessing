    ...
    private val defaultConfig = """
                ...
                packet-limiter {
                    ...
                    proxy {
                        ...
                        size-limit=4096
                        # 测量发送数据包的间隔.
                        # 越小 (大于-1): 将更快检测到恶意发送多个大量数据包的连接, 但可能会意外切断连接质量不稳定的玩家的连接
                        # 较大: 更宽松的检测, 允许连接质量不稳定的玩家游玩而不会被意外切断连接. 但同时可能会有更多的漏洞利用
                        interval=7.0
                        # 玩家在interval配置的间隔内的最大数据包数.
                    ...
                    # 如果触发了数据包限制器并抛出了异常, 是否由数据包限制器本身忽略抛出的异常并关闭连接?
                    proxy-silent-exception=true
                }
    ...
    private val defaultMessage = """
                ...
                statistics {
                    ...
                    actionbar-format {
                        ...
                        attack="%prefix% <aqua>Type %type%  <aqua>CPS <white>%cps%  <aqua>Peak <white>%peak_cps_session%  <aqua>Total <white>%total_session%  <aqua>Blocked <white>%blocked_session%  <aqua>CPU <white>%process_cpu%%  <aqua>Duration <white>%duration%  <red>↑ %outgoing-bytes%  <green>↓ %incoming-bytes%"
                    }
                    chat-format {
                        idle=[
                            "",
