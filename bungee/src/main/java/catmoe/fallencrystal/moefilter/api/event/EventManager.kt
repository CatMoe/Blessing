...
object EventManager {
    ...
    private val plugin = MoeFilterBungee.instance as Plugin
    ...
    private fun methodTrigger(listener: EventListener, event: Any) {
        ...
        if (event is MoeEvent) { methods.forEach { if (needSend(event, it)) { try { it.invoke(listener, it) } catch (e: Exception) { e.printStackTrace() } } } }
    }
